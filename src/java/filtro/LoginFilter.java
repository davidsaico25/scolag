...
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        // Obtengo el usuario guardado en session       
        Usuario usuario = (Usuario) req.getSession().getAttribute("usuario");
        Boolean isLogged = (Boolean) req.getSession().getAttribute("logged");
        
        //Proceso la URL que está requiriendo el cliente
        String url = req.getRequestURL().toString();
        
        if (needsExtension(url)) {
            url += ".xhtml";
        ...
        if (!restrict(url)) {
            if (usuario != null && isLogged && url.endsWith("/scolag/login.xhtml")) {
    ...
    private boolean needsExtension(String url) {
        return !url.endsWith(".xhtml") && !url.endsWith("/") && !url.contains("/scolag/javax.faces.resource/") && !url.contains("/scolag/resources/");
    }
    ...
    private boolean restrict(String urlStr) {
         ...
         * compilación.
         */
        List<String> listURLAllow = new ArrayList<>();
        listURLAllow.add("scolag/");
        listURLAllow.add("scolag/login.xhtml");
        for (String url : listURLAllow) {
            if (urlStr.endsWith(url)) {
                return false;
            }
        }
        
        List<String> listResourceAllow = new ArrayList<>();
        listResourceAllow.add("/scolag/resources/css/");
        listResourceAllow.add("/scolag/resources/js/");
        listResourceAllow.add("/scolag/javax.faces.resource/");
        for (String resource : listResourceAllow) {
            if (urlStr.contains(resource)) {
                return false;
            }
        }
        return true;
